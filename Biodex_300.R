...
biodex_300 <- read_excel("Biodex Test Data.xlsx", sheet = "Biodex_300")
View(biodex_300)

biodex_300  <- clean_names(biodex_300)


#---- Data wrangling ----

ggplot(biodex_300  , aes(x = test, y = right_peak_ext_torque)) +
...
biodex_300 <- biodex_300 %>%
  mutate(across(-c(id, test), as.numeric))

peak_ext_300_wide <- biodex_300 %>%
...
view(peak_ext_300_wide)

peak_ext_300_wide <- peak_ext_300_wide %>%
  filter(id != "ETC 86")
view(peak_ext_300_wide)
...
peak_ext_300_summary <- peak_ext_300_wide %>%
  summarise(across(where(is.numeric), 
                   list(mean = ~mean(.x, na.rm = TRUE), 
                        sd = ~sd(.x, na.rm = TRUE))))

# View results
view(peak_ext_300_summary)

peak_flex_300_wide <- biodex_300 %>%
...
view(peak_flex_300_wide)

# Calculate mean and SD for each numeric column
peak_flex_300_summary <- peak_flex_300_wide %>%
  summarise(across(where(is.numeric), 
                   list(mean = ~mean(.x, na.rm = TRUE), 
                        sd = ~sd(.x, na.rm = TRUE))))

# View results
view(peak_flex_300_summary)

t.test(
  peak_ext_300_wide$`right_peak_ext_torque_Test 2`,
  peak_ext_300_wide$`right_peak_ext_torque_Test 1`,
  paired = TRUE
)
...
icc_biodex_300 <- peak_ext_300_wide %>%
  select(`right_peak_ext_torque_Test 2`, `right_peak_ext_torque_Test 1`)
icc_result <- ICC(icc_biodex_300)
print(icc_result)

# Replace these column names with your actual dataset
test1 <- peak_ext_300_wide$`right_peak_ext_torque_Test 1`
test2 <- peak_ext_300_wide$`right_peak_ext_torque_Test 2`
...
ggplot(peak_ext_300_wide, aes(x = `right_peak_ext_torque_Test 2`, 
...
t.test(
  peak_ext_300_wide$`left_peak_ext_torque_Test 2`,
  peak_ext_300_wide$`left_peak_ext_torque_Test 1`,
  paired = TRUE
)
...
# Replace these column names with your actual dataset
test1 <- peak_ext_300_wide$`left_peak_ext_torque_Test 1`
test2 <- peak_ext_300_wide$`left_peak_ext_torque_Test 2`
...
t.test(
  peak_flex_300_wide$`right_peak_flex_torque_Test 2`,
  peak_flex_300_wide$`right_peak_flex_torque_Test 1`,
  paired = TRUE
)
...
icc_biodex_300 <- peak_flex_300_wide %>%
  select(`right_peak_flex_torque_Test 2`, `right_peak_flex_torque_Test 1`)
icc_result <- ICC(icc_biodex_300)
print(icc_result)

# Replace these column names with your actual dataset
test1 <- peak_flex_300_wide$`right_peak_flex_torque_Test 1`
test2 <- peak_flex_300_wide$`right_peak_flex_torque_Test 2`
...
ggplot(peak_flex_300_wide, aes(x = `right_peak_flex_torque_Test 2`, 
...
t.test(
  peak_flex_300_wide$`left_peak_flex_torque_Test 2`,
  peak_flex_300_wide$`left_peak_flex_torque_Test 1`,
  paired = TRUE
)
...
# Replace these column names with your actual dataset
test1 <- peak_flex_300_wide$`left_peak_flex_torque_Test 1`
test2 <- peak_flex_300_wide$`left_peak_flex_torque_Test 2`
...
y_min <- min(c(min(biodex_300$right_peak_ext_torque), min(biodex_300$left_peak_ext_torque),
               min(biodex_300$right_peak_flex_torque), min(biodex_300$left_peak_flex_torque)))
y_max <- max(c(max(biodex_300$right_peak_ext_torque), max(biodex_300$left_peak_ext_torque),
               max(biodex_300$right_peak_flex_torque), max(biodex_300$left_peak_flex_torque)))


# Right max extension torque
p1_300 <- ggplot(biodex_300, aes(x = test, y = right_peak_ext_torque)) +
  geom_boxplot(aes(fill = test), alpha = 0.5) +
  geom_jitter(width = 0.2, aes(color = test), size = 2) +
...
p2_300 <- ggplot(biodex_300, aes(x = test, y = left_peak_ext_torque)) +
  geom_boxplot(aes(fill = test), alpha = 0.5) +
  geom_jitter(width = 0.2, aes(color = test), size = 2) +
...
p3_300 <- ggplot(biodex_300, aes(x = test, y = right_peak_flex_torque)) +
  geom_boxplot(aes(fill = test), alpha = 0.5) +
  geom_jitter(width = 0.2, aes(color = test), size = 2) +
...
p4_300 <- ggplot(biodex_300, aes(x = test, y = left_peak_flex_torque)) +
  geom_boxplot(aes(fill = test), alpha = 0.5) +
  geom_jitter(width = 0.2, aes(color = test), size = 2) +
...
# Arrange plots 2x2 with overall title
library(patchwork)

(p1_300 | p2_300) / (p3_300 | p4_300) +
  plot_annotation(
    title = "Biodex 300°/Second",
